...
class VOC_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        xml_path = os.path.join(self.label_root, xml_name)
        image_mark = self.parse_xml(xml_path)
        # print(image_mark)
        # self.draw_mark(image_array, image_mark)
        
        # data augmentation
        if self.txt_file.endswith('train.txt') or self.txt_file.endswith('trainval.txt'):
    ...
    def parse_xml(self, xml_path):
        ...
        for object in objects:
            ob_name = object.find('name').text
            ob_id = self.class_list.index(ob_name)
            ob_points = object.find('bndbox')
            ob_xmin = int(ob_points.find('xmin').text)
            ob_ymin = int(ob_points.find('ymin').text)
            ob_xmax = int(ob_points.find('xmax').text)
            ob_ymax = int(ob_points.find('ymax').text)
...
if __name__ == '__main__':
    ...
    train_ds = VOC_Dataset(image_root='VOC2007\JPEGImages',
                        label_root='VOC2007\Annotations',
                        txt_file='VOC2007\ImageSets\Main\\train.txt',
                        transform=transform)
    
    val_ds = VOC_Dataset(image_root='VOC2007\JPEGImages',
                        label_root='VOC2007\Annotations',
                        txt_file='VOC2007\ImageSets\Main\\val.txt',
                        transform=transform)
